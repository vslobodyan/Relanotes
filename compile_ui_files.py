...
some_paths_to_ui_files = []

relative_paths = [ 'relanotes/qtdesign_ui/', 'relacalc/qtdesign_ui/' ]

for a_relative_path in relative_paths:
    path_to_ui_files = os.path.join(path_to_app, a_relative_path)
    path_to_ui_files = give_correct_path_under_win_and_other(path_to_ui_files)
    some_paths_to_ui_files.append(path_to_ui_files)
...
def walkthrough_path_and_compile_ui_files(path_to_ui_files):
...
# Проделываем операцию для всех указанных каталогов, где есть ui-файлы
for a_path_to_ui_files in some_paths_to_ui_files:
    walkthrough_path_and_compile_ui_files(a_path_to_ui_files)
