...
Перейти в поиск из заметки: Ctrl+J
Вы сразу окажетесь в поле поиска.
Наберите слово с пробелом в начале для поиска по содержимому заметок:
...
Слово без пробела - поиск по названию (или пути) заметок:
 * ```proj``` найдет все заметки с именами projects, proj. и т.д.

 Если поставить 2 пробела в начале, то произойдет поиск по всем заметкам, в которых указанному слову предшествует пробел.
