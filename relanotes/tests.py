...
class App_Tests():
    ...
    def collect_items(self, from_directory=False, change_path = False, from_notelist=False):
        ...
        self.items = []
        if from_notelist:
            for one_item in self.rn_app.notelist.items:
        ...
        if from_directory:
            ...
            if change_path or not self.path_to_notes_convertation:
                ...
                new_path = give_correct_path_under_win_and_other(QtWidgets.QFileDialog.getExistingDirectory(self.rn_app.main_window, "Select Directory with your Notes for Test", '' , QtWidgets.QFileDialog.ShowDirsOnly))
            ...
            for root, dirs, files in os.walk(self.path_to_notes_convertation):
                for file in files:
                    if os.path.splitext(file)[-1] in self.rn_app.notelist.allowed_note_files_extensions:
    ...
    def update_path_info_for_notes_convertation(self, new_path, save=False):
        ...
        if self.path_to_notes_convertation:
            self.rn_app.main_window.actionRun_test_for_notes_convertation_in_last_directory.setText('Run test for notes convertation in %s' % self.path_to_notes_convertation)
        else:
            self.rn_app.main_window.actionRun_test_for_notes_convertation_in_last_directory.setText('Run test for notes convertation in last directory')
        if save:
            # Сохраняем переменную с новым каталогом
            self.rn_app.app_settings.settings.setValue('path_to_notes_convertation_test', self.path_to_notes_convertation)
            self.rn_app.app_settings.settings.sync()
    ...
    def test_notes_items_for_health_bad_link(self):
        ...
        for filename in self.items:
            ...
            self.rn_app.note.health_bad_links(filename, original_text)
    ...
    def test_notes_items_for_convertation(self):
        ...
        for filename in self.items:
            ...
            self.rn_app.main_window.open_file_in_editor(filename, dont_save_in_history=True)

            # Конвертируем zim text в html для редактора
            html_source = self.rn_app.note.convert_zim_text_to_html_source(original_text)
            # Устанавливаем html-исходник для редактора
            self.rn_app.main_window.doc_source.setHtml(html_source)
            self.rn_app.main_window.textBrowser_Note.setDocument(self.rn_app.main_window.doc_source)


            # Конвертируем файл как-бы для сохранения на диск
            note_source = self.rn_app.main_window.textBrowser_Note.toHtml()
            saved_text = self.rn_app.note.convert_html_source_to_zim_text(note_source)
    ...
    def initial_setup(self):
        ...
        self.rn_app.main_window.actionRun_test_notes_convertation_for_notelist.triggered.connect(self.notes_convertation_for_notelist)

        self.rn_app.main_window.actionSelect_another_directory_and_run_test_for_notes_convertation.triggered.connect(self.notes_convertation_change_path)

        self.rn_app.main_window.actionRun_test_health_bad_links_for_notes_from_notelist.triggered.connect(self.health_bad_links_for_notes_from_notelist)
