...
class Main_Window(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    # # Упоминания переменных, которые мы будем использовать в работе.
    # rn_app..app_settings = None
    # .notelist = None
    # .main_window = None
    ...
    def check_path_to_notes_or_select_new(self, select_new=False):
        ...
        if not self.rn_app.settings.path_to_notes:
...
Включенные расширения файлов: %s''' % self.rn_app.notelist.allowed_note_files_extensions
        ...
        elif not os.path.exists(self.rn_app.settings.path_to_notes):
...
Открыть другой каталог с заметками?''' % self.rn_app.settings.path_to_notes
            reply = QtWidgets.QMessageBox.question(self,
                                                   message_title,
                                                   message_text,
        ...
        if select_new:
            ...
            self.rn_app.settings.path_to_notes = give_correct_path_under_win_and_other(QtWidgets.QFileDialog.getExistingDirectory(self, "Select Directory with your Notes", '' , QtWidgets.QFileDialog.ShowDirsOnly))
            self.rn_app.settings.settings.setValue('path_to_notes', self.rn_app.settings.path_to_notes)
            self.rn_app.settings.settings.sync()
            print("Выбран новый путь к заметкам:", self.rn_app.settings.path_to_notes)
    ...
    def initial_setup(self):
        ...
        try:
            ...
            self.restoreGeometry(self.rn_app.settings.settings.value("mainWindow/geometry"))
            self.restoreState(self.rn_app.settings.settings.value("mainWindow/windowState"))
        except:
            pass
    ...
    def __init__(self, rn_app, parent=None):
        ...
        # self..main_window = rn_app..main_window
        # self..clear_history_win = rn_app..clear_history_win
        # self..calculator_win = rn_app..calculator_win
        # self..preferences_win = rn_app..preferences_win
        # self.note = rn_app.note
        # self.root_logger = rn_app.loggers.root
        # self.text_format = rn_app.text_format
    ...
    def notelist_filter_cursorPositionChanged(self, old, new):
        ...
        if self.rn_app.notelist.filter_in_change:
            # Если производятся внутренние манипуляции с полем фильтра- ждем их окончания
            return 0

        if self.rn_app.notelist.filter_is_empty:
        ...
        if ' ' not in left_part:
            ...
            if right_part:
                ...
                tip_text_html = self.rn_app.notelist.filter_editing_tips['name']
            else:
                # Правой части фильтра от курсора на имени нет
                tip_text_html = self.rn_app.notelist.filter_editing_tips['empty']
        else:
            # Курсор стоит на тексте
            tip_text_html = self.rn_app.notelist.filter_editing_tips['text']

        tip_html = '%s%s%s' % (self.rn_app.notelist.filter_editing_tips['html_begin'],
                                tip_text_html,
                               self.rn_app.notelist.filter_editing_tips['html_end'])
    ...
    def notelist_filter_changed(self, filter_text):
        ...
        self.rn_app.notelist.cancel_scheduled_update()
        self.lineNotelist_Filter.setFocus()
        ...
        new_filter_name, new_filter_text = self.rn_app.notelist.extract_filters(filter_text)
        if new_filter_name == self.rn_app.notelist.filter_name and new_filter_text == self.rn_app.notelist.filter_text:
        ...
        if not filter_text:
            ...
            self.rn_app.notelist.filter_in_change = True
            self.lineNotelist_Filter.setText(self.rn_app.notelist.filter_tip_for_using)
            self.lineNotelist_Filter.setStyleSheet('''
                                color: #aaa;
                                font-size: 14px;
                                background: white;
                                '''
            ...
            self.rn_app.notelist.filter_in_change = False
            ...
            if self.rn_app.notelist.filter_is_empty:
                ...
                self.rn_app.notelist.filter_is_empty = True
                ...
                if needed_list_update:
                    self.rn_app.notelist.schedule_update()
                return 0
        else:
            ...
            if not ( self.rn_app.notelist.filter_is_empty and filter_text == self.rn_app.notelist.filter_tip_for_using):
                ...
                if self.rn_app.notelist.filter_is_empty and filter_text.endswith(self.rn_app.notelist.filter_tip_for_using):
                    ...
                    cleared_user_filter = filter_text.rpartition(self.rn_app.notelist.filter_tip_for_using)[0]
                    #print('text after rpartition: #%s#' % cleared_user_filter)
                    self.lineNotelist_Filter.setText(cleared_user_filter)
                    ...
                    self.rn_app.notelist.filter_is_empty = False
                    if needed_list_update:
                        self.rn_app.notelist.schedule_update()
                    return 0
        ...
        if self.rn_app.notelist.filter_is_empty:
            ...
            if filter_text == self.rn_app.notelist.filter_tip_for_using:
                ...
                self.rn_app.notelist.filter_is_empty = False
                self.rn_app.notelist.filter_in_change = True
                self.lineNotelist_Filter.setText(filter_text)
                self.lineNotelist_Filter.setStyleSheet('''
                                    color: #1a1a1a;
                                    font-size: 16px;
                                    background: #fff8a5;
    ...
    def save_note_cursor_position(self):
        ...
        filename = self.current_open_note_link
        if filename:
            current_position = self.textBrowser_Note.textCursor().position()
            ...
            self.rn_app.settings.state_db_connection.execute("UPDATE file_recs SET current_position=?  WHERE filename=?",
    ...
    def closeEvent(self, e):
        ...
        self.rn_app.settings.settings.setValue("mainWindow/geometry", self.saveGeometry())
        self.rn_app.settings.settings.setValue("mainWindow/windowState", self.saveState())
        self.rn_app.settings.settings.sync()
    ...
    def history_clear(self):
        ...
        self.rn_app.clear_history_win.history_items = []

        #layout = QtWidgets.QVBoxLayout(.clear_history_win.scrollArea)
        #layout.setAlignment(QtCore.Qt.AlignTop)

        layout = QtWidgets.QVBoxLayout(self.rn_app.clear_history_win.scrollAreaWidgetContents)
        layout.setAlignment(QtCore.Qt.AlignTop)
        ...
        for row in file_recs_rows:
            ...
            history_item = self.rn_app.clear_history_win.history_rec.copy()
            ...
            self.rn_app.clear_history_win.history_items.append(history_item)
        ...
        if self.rn_app.clear_history_win.exec():
            print('Надо удалить из истории:')
            for one_item in self.rn_app.clear_history_win.history_items:
                if one_item['checkbox'].isChecked():
                    print(' - %s' % one_item['filename'])
                    self.rn_app.settings.state_db_connection.execute("UPDATE file_recs SET last_open=NULL, count_opens=0 WHERE filename=?", (one_item['filename'],))
                    self.rn_app.notelist.update(history_update=True)
    ...
    def initial_db(self):
        ...
        try:
            self.rn_app.settings.state_db_connection.execute('''CREATE TABLE file_recs
    ...
    def show_calculator(self):
        self.rn_app.calculator_win.show()
        # calculator_form.show()

    def show_preferences(self):
        self.rn_app.preferences_win.show()
    ...
    def renew_history_lists(self, active_link=None):
        ...
        html_string = self.rn_app.notelist.make_html_source_for_items_list_in_history_sidebar()
    ...
    def extract_real_styles_note_format(self):
        ...
        test_note_source = self.rn_app.note.convert_zim_text_to_html_source(testnote)
        ...
        def next_span_style(pos, str='', skip_next=False, log=True, comment=''):
            ...
            if log:
                self.rn_app.loggers.root.info('Извлечение стиля %s из span%s,  pos %s-%s, "%s"' % (comment, skip_next_notify, begin_pos, end_pos, style))
            else:
                pass
        ...
        self.rn_app.text_format.editor_default_font_span = span_template % style_default
        self.rn_app.text_format.editor_h1_span = span_template % style_h1
        self.rn_app.text_format.editor_h2_span = span_template % style_h2
        self.rn_app.text_format.editor_h3_span = span_template % style_h3
        self.rn_app.text_format.editor_h4_span = span_template % style_h4
        self.rn_app.text_format.editor_h5_span = span_template % style_h5
        self.rn_app.text_format.editor_h6_span = span_template % style_h6

        self.rn_app.text_format.editor_code_span = span_template % style_code
        self.rn_app.text_format.editor_strikethrough_span = span_template % style_strike
        self.rn_app.text_format.editor_mark_span = span_template % style_mark
        self.rn_app.text_format.editor_link_external_style = style_link #move_down_and_get_span(test_cursor, only_style=True)
        ...
        self.rn_app.text_format.editor_h_span = ['0-empty', self.rn_app.text_format.editor_h1_span,
                                                 self.rn_app.text_format.editor_h2_span,
                                                 self.rn_app.text_format.editor_h3_span,
                                                 self.rn_app.text_format.editor_h4_span,
                                                 self.rn_app.text_format.editor_h5_span,
                                                 self.rn_app.text_format.editor_h6_span]
        ...
        self.rn_app.loggers.root.info(self.textBrowser_TestNote.toHtml())
    ...
    def open_file_in_editor(self, filename, line_number=None, found_text=None, reload=False, dont_save_in_history=False):
        ...
        if reload:
            ...
            if self.rn_app.notelist.file_in_state_db(filename):
                # Запись о файле уже есть. Получаем из неё последнюю позицию
                self.rn_app.settings.state_db_connection.execute("SELECT count_opens, current_position FROM file_recs WHERE filename=?", (filename,))
                rec_count_opens, rec_current_position = self.rn_app.settings.state_db_connection.fetchone()
        ...
        elif self.rn_app.notelist.is_visible():
            # rec = [ 'note' / 'list', 'filename' / 'filter', datetime ]
            if self.rn_app.notelist.history_position == 0:
                new_recs_sel = []
                ...
                if self.rn_app.notelist.file_in_state_db(filename):
                    ...
                    if not dont_save_in_history:
                        # Обновляем запись в базе
                        self.rn_app.settings.state_db_connection.execute("UPDATE file_recs SET last_open=?, count_opens=?  WHERE filename=?",
                    ...
                    if not dont_save_in_history:
                        self.rn_app.settings.state_db_connection.execute("INSERT INTO file_recs (filename, last_open, count_opens) VALUES (?,?,?)",
                                                        (filename, datetime.now(), 1))
                        self.rn_app.settings.state_db.commit()

        if not dont_save_in_history:
            # Добавляем элемент к отдельному списку истории
            self.rn_app.notelist.update_history_items_with_one(filename)
        ...
        note_source = self.rn_app.note.convert_zim_text_to_html_source(lines)
        ...
        self.rn_app.loggers.root.info('=' * 40)
        self.rn_app.loggers.root.info(self.textBrowser_Note.toHtml())
        # print('Получили для адаптации при вставке следующий html:\n' + html_source + '\n')
        self.rn_app.loggers.root.info('=' * 40)
        ...
        if not line_number == None:  # Используем -1 а не None - чтобы Питон не делал из None-переменной 0
            ...
            lines_to_skip = int(line_number)-len(self.rn_app.note.metadata_lines_before_note)-1
            ...
            self.textBrowser_Note.ensureCursorVisible()
        ...
        file_cute_name = self.rn_app.notelist.make_cute_name(filename)

        self.setWindowTitle(self.rn_app.settings.Name + ' - ' + file_cute_name)
        ...
        #cursor = self.textBrowser_Note.textCursor()
        pos1 = cursor.position()
        line_current = self.rn_app.text_format.getLineAtPosition3(pos1)

        cursor.movePosition(QtGui.QTextCursor.End)
        pos2 = cursor.position()
        lines_all = self.rn_app.text_format.getLineAtPosition3(pos2)

        #print('Информация для расчета изменения промотки: pos1 %s / line_current %s, pos2 %s / lines_all %s' % (pos1, line_current, pos2, lines_all))

        self.adjust_scrollbar_position_at_editor(self.textBrowser_Note, line_current, lines_all)
    ...
    def loadfile2(self, url):
        ...
        # в списке истории.
        self.rn_app.notelist.history_position = 0
    ...
    def copy_snippet_text_to_clipboard(self, number):
        ...
        text = self.rn_app.settings.snippet_actions[number].statusTip()
    ...
    def edit_snippets(self):
        fname = self.rn_app.settings.snippets_filename
    ...
    def show_snippets(self):
        ...
        if os.path.isfile(fname):
            ...
            snippets = total_text.split(self.rn_app.settings.snippets_separator)
            ...
            self.rn_app.settings.snippet_actions = []
            ...
            for snippet in snippets:
                ...
                self.rn_app.settings.snippet_actions.append(
                    QtWidgets.QAction(QtGui.QIcon('clipboard.png'), snippet_name, self.menuSnippets) )
                self.rn_app.settings.snippet_actions[snippet_ndx].setStatusTip(snippet_text)
                ...
                self.rn_app.settings.snippet_actions[snippet_ndx].triggered.connect(lambda arg, snippet_ndx=snippet_ndx: self.copy_snippet_text_to_clipboard(snippet_ndx))

                self.menuSnippets.addAction(self.rn_app.settings.snippet_actions[snippet_ndx])
            ...
            self.rn_app.settings.snippet_actions.append(QtWidgets.QAction(
                QtGui.QIcon('edit.png'), 'Редактирование сниппетов..', self.menuSnippets))

            self.rn_app.settings.snippet_actions[-1].triggered.connect(
                self.edit_snippets )
            self.menuSnippets.addAction(self.rn_app.settings.snippet_actions[-1])
